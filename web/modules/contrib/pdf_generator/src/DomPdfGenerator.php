...
class DomPdfGenerator {
   ...
   */
  public function postResponse($title, array $content, $preview = FALSE, array $options = [], $pageSize = 'A4', $disposition = 'portrait', $cssText = NULL, $cssPath = NULL) {
    ...
    $filename = strtolower(trim(preg_replace('#\W+#', '', $title), '')).'_'.date('Y_m_d_h_i_s');
    $response->headers->set('Content-Disposition', "attachment; filename={$filename}.pdf");

    if (file_exists($_SESSION['pdfroot'])) {
      $ruta = $_SESSION['pdfroot'];
      $success = unlink($ruta);
    }

    $directory = 'public://private/pdf';
    file_prepare_directory($directory, FILE_CREATE_DIRECTORY);

    $directory = \Drupal::service('stream_wrapper_manager')
      ->getViaUri('public://private/pdf')
      ->realpath();
    

    $pdfroot = $directory . '/' . "$filename.pdf";
    $_SESSION['pdfroot'] = $pdfroot;
    $_SESSION['salvapdfroot'] = $pdfroot;
    ...
    return [
      'path' => $pdfroot,
      'name' => $filename . '.pdf',
      'browser_path' => '/sites/default/files/private/pdf/' . $filename . '.pdf'
    ];
  }
